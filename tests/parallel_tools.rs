...
#[derive(Debug, Serialize, Deserialize, JsonSchema)]
struct UnknownParallelIn {
    text: String,
}
#[derive(Debug, Serialize, Deserialize, JsonSchema)]
struct UnknownParallelOut {
    summary: String,
}
simple_agent!(
    UnknownParallelIn,
    UnknownParallelOut,
    "Use echo and reverse."
);
...
async fn test_two_distinct_tools_run_in_parallel() {
    ...
    let rt = FlowRuntime::new(ParallelIn {
        text: "test".into(),
    })
    .unwrap()
...
async fn test_unknown_tool_alongside_known_gets_error_result() {
    ...
    let spy = factory.clone();
    let rt = FlowRuntime::new(UnknownParallelIn {
        text: "test".into(),
    })
    .unwrap()
    .with_factory(factory);
...
async fn test_llm_error_on_re_dispatch_propagates_as_agent_error() {
    ...
    match err {
        FlowError::Agent(AgentError::LlmFailed { reason, .. }) => {
            assert!(
                reason.contains("second call failed"),
                "unexpected reason: {reason}"
            );
        }
        other => panic!("expected LlmFailed, got: {other}"),
