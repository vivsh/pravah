...
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct DirectIn { prompt: String }
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct DirectOut { answer: String }
...
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct ValidIn { query: String }
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct ValidOut { result: String }
...
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct MultiToolIn { text: String }
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct MultiToolOut { summary: String }
...
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct DupIn { text: String }
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct DupOut { result: String }
...
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct UnknownIn { text: String }
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct UnknownOut { result: String }
...
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct HistoryIn { q: String }
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct HistoryOut { a: String }
...
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct ChainIn { start: String }
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct ChainOut { final_value: String }
...
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct LlmErrIn { x: String }
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct LlmErrOut { y: String }
...
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct StructModeIn { q: String }
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct StructModeOut { answer: String }
...
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct StoreIn { text: String }
#[derive(Debug, Serialize, Deserialize, JsonSchema)] struct StoreOut { echoed: String }
...
async fn test_inspector_tracks_tool_turns() {
    ...
    match top.agent_phases.first().map(|ap| &ap.phase) {
        Some(PhaseKind::PendingTool { active_calls, waiting_count }) => {
    ...
    loop {
        match rt.next(ctx()).await.unwrap() {
            FlowStep::Done(out) => { assert_eq!(out.result, "echoed:hello"); break; }
            FlowStep::Continue => {}
...
async fn test_multiple_tool_calls_in_one_turn() {
    ...
    let rt = FlowRuntime::new(MultiToolIn { text: "hi".into() }).unwrap().with_factory(factory);
    let out = run_to_done(rt).await.unwrap();
...
async fn test_same_tool_twice_runs_serially() {
    ...
    let rt = FlowRuntime::new(DupIn { text: "test".into() }).unwrap().with_factory(factory);
    let out = run_to_done(rt).await.unwrap();
...
async fn test_duplicate_call_id_is_rejected() {
    ...
    assert!(matches!(err, FlowError::Agent(AgentError::DuplicateToolCall { .. })));
}
...
async fn test_unknown_tool_name_becomes_error_result() {
    ...
    let rt = FlowRuntime::new(UnknownIn { text: "test".into() }).unwrap().with_factory(factory);
    let out = run_to_done(rt).await.unwrap();
...
async fn test_tool_result_present_in_second_dispatch() {
    ...
    let spy = factory.clone();
    let rt = FlowRuntime::new(HistoryIn { q: "ping?".into() }).unwrap().with_factory(factory);
    ...
    assert!(second.iter().any(|m| matches!(&m.role, Role::Tool { call_id } if call_id == "tc1")));
    let atc_pos = second.iter().position(|m| matches!(&m.role, Role::AssistantToolCalls { .. })).unwrap();
    let tool_pos = second.iter().position(|m| matches!(&m.role, Role::Tool { call_id } if call_id == "tc1")).unwrap();
...
async fn test_both_tool_results_present_after_multi_call_turn() {
    ...
    let spy = factory.clone();
    let rt = FlowRuntime::new(MultiToolIn { text: "test".into() }).unwrap().with_factory(factory);
    run_to_done(rt).await.unwrap();
    let second = &spy.calls()[1].1;
    for call_id in ["id1", "id2"] {
        assert!(second.iter().any(|m| matches!(&m.role, Role::Tool { call_id: cid } if cid == call_id)));
    }
}
...
async fn test_three_sequential_tool_call_rounds() {
    ...
    let spy = factory.clone();
    let rt = FlowRuntime::new(ChainIn { start: "go".into() }).unwrap().with_factory(factory);
    let out = run_to_done(rt).await.unwrap();
...
async fn test_tool_results_accumulate_across_rounds() {
    let factory = ScriptedFactory::new()
        ...
        .then_output(json!({ "final_value": "done" }));
    let spy = factory.clone();
    let rt = FlowRuntime::new(ChainIn { start: "chain".into() }).unwrap().with_factory(factory);
    run_to_done(rt).await.unwrap();
    let calls = spy.calls();
    let t2 = &calls[1].1;
    assert!(t2.iter().any(|m| matches!(&m.role, Role::Tool { call_id } if call_id == "r1")));
    let t3 = &calls[2].1;
    for id in ["r1", "r2"] {
        assert!(t3.iter().any(|m| matches!(&m.role, Role::Tool { call_id } if call_id == id)));
    }
}
...
async fn test_llm_error_on_first_dispatch_propagates() {
    ...
    let rt = FlowRuntime::new(LlmErrIn { x: "hello".into() }).unwrap().with_factory(factory);
    let err = run_to_err(rt).await;
    match err {
...
async fn test_direct_output_in_tool_mode_completes_agent() {
    ...
    let spy = factory.clone();
    let rt = FlowRuntime::new(StructModeIn { q: "quick?".into() }).unwrap().with_factory(factory);
    let out = run_to_done(rt).await.unwrap();
...
async fn test_capturing_store_receives_all_messages() {
    ...
    let rt = FlowRuntime::new(StoreIn { text: "world".into() })
        .unwrap().with_factory(factory).with_store(store);
    run_to_done(rt).await.unwrap();
    ...
    assert!(live.iter().any(|e| matches!(&e.message.role, Role::AssistantToolCalls { .. })));
    assert!(live.iter().any(|e| matches!(&e.message.role, Role::Tool { call_id } if call_id == "s1")));
}
...
async fn test_no_scripted_responses_remain_after_valid_run() {
    let factory = ScriptedFactory::new()
        ...
        .then_output(json!({ "result": "done" }));
    let spy = factory.clone();
    let rt = FlowRuntime::new(ValidIn { query: "consume all".into() }).unwrap().with_factory(factory);
    run_to_done(rt).await.unwrap();
    assert_eq!(spy.remaining(), 0);
}
...
async fn test_no_scripted_responses_remain_after_direct_run() {
    ...
    let spy = factory.clone();
    let rt = FlowRuntime::new(DirectIn { prompt: "any?".into() }).unwrap().with_factory(factory);
    run_to_done(rt).await.unwrap();
    assert_eq!(spy.remaining(), 0);
}
