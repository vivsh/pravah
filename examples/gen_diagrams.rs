...
impl Agent for OutlineRequest {
    ...
    fn configure() -> AgentConfig {
        AgentConfig::new(
            "Generate a structured outline with sections.",
...
impl Agent for LongDraft {
    ...
    fn configure() -> AgentConfig {
        AgentConfig::new(
            "Review the draft for quality, accuracy, and structure.",
            "gemini:///gemini-2.5-flash-lite",
        )
    }
}
...
impl Agent for ResearchTask {
    ...
    fn configure() -> AgentConfig {
        AgentConfig::new(
            "Research the topic and gather key facts and sources.",
            "gemini:///gemini-2.5-flash-lite",
        )
    }
}
...
impl Agent for AudienceTask {
    ...
    fn configure() -> AgentConfig {
        AgentConfig::new(
            "Analyse the target audience and determine tone and reading level.",
            "gemini:///gemini-2.5-flash-lite",
        )
    }
}
...
impl Agent for QuickDraft {
    ...
    fn configure() -> AgentConfig {
        AgentConfig::new(
            "Write a concise, punchy article from the quick draft.",
            "gemini:///gemini-2.5-flash-lite",
        )
    }
}
...
impl Agent for ReviewedDraft {
    ...
    fn configure() -> AgentConfig {
        AgentConfig::new(
            "Polish the reviewed draft into a publication-ready article.",
            "gemini:///gemini-2.5-flash-lite",
        )
    }
}
...
fn merge_findings(research: ResearchNotes, audience: AudienceProfile) -> ContentBrief {
    ContentBrief {
        notes: research.notes,
        tone: audience.tone,
        format: audience.reading_level,
    }
}
...
async fn prepare_outline(brief: ContentBrief, _ctx: Context) -> Result<OutlineRequest, FlowError> {
    Ok(OutlineRequest {
        brief: format!(
            "notes={} tone={} format={}",
            brief.notes, brief.tone, brief.format
        ),
    })
}
        ...
        match rt.next(ctx.clone()).await? {
            ...
            FlowStep::Suspend(_) => {
                return Err(FlowError::Internal {
                    handler: "gen_diagrams",
                    detail: "outline flow suspended unexpectedly".into(),
                });
            }
        }
    }
...
fn route_draft(outline: Outline) -> Either<QuickDraft, LongDraft> {
    if outline.format == "quick" || outline.sections.len() <= 3 {
        Either::Left(QuickDraft {
            content: outline.sections.join("\n"),
        })
    } else {
        Either::Right(LongDraft {
    ...
    loop {
        match rt.next(ctx.clone()).await? {
            ...
            FlowStep::Suspend(_) => {
                return Err(FlowError::Internal {
                    handler: "gen_diagrams",
                    detail: "review flow suspended unexpectedly".into(),
                });
            }
        }
    }
...
impl Flow for ArticleRequest {
    ...
    fn build(root: Node<Self>) -> Node<Self::Output> {
        ...
        research
            .agent()
            .merge(audience.agent(), |(research, audience)| {
                merge_findings(research, audience)
            })
