...
/// at runtime using `build` and drives it via [`crate::legacy::FlowRuntime`].
///
/// # Panics
///
/// If `build` is not overridden, any attempt to compile the flow
...
pub trait Flow: 'static + JsonSchema + Serialize + DeserializeOwned + Send + Sync {
    ...
    ///
    /// `root` is a pre-typed entry node for `Self`. Chain node methods on it
    /// until the final expression has type `Node<Self::Output>`.
    fn build(_root: Node<Self>) -> Node<Self::Output> {
        unimplemented!(
            "`{}` must implement `Flow::build`",
...
async fn record_history(
    ...
    let entry = history.prepare_entry(session_id, agent_id, message);
    store
        .record_dyn(&entry)
        .await
        .map_err(|err| FlowError::History {
            operation: "record",
...
impl FlowGraph {
    ...
    pub(crate) async fn resume(
        ...
        self.step(services, ctx, history, Some(resumption), states)
            .await
    }
    ...
    async fn handle_agent(
        ...
        match current {
            None => {
                ...
                record_history(services.store, history, &session_id, agent_name, message).await?;
    ...
    async fn handle_pending_tools(
        ...
        for (exit_id, call_id) in completions {
            ...
            record_history(store, history, &session_id, agent_name, msg).await?;
...
impl FlowGraph {
        ...
        for state_index in 0..total_states {
            ...
            match current_node {
                FlowNode::Agent(agent) => {
                    ...
                    return Self::handle_agent(agent, self, services, ctx, history, states).await;
                }
    ...
    async fn step(
        ...
        let result = self.step_inner(services, ctx, history, states).await?;
        match result {
            FlowStep::Continue => {
